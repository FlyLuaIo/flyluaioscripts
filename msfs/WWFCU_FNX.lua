...
GlobalFrameLoopManager:add(function()
	...
	local speed, heading, altitude, vs = displayValues.N_FCU_SPEED, displayValues.N_FCU_HEADING,
		displayValues.N_FCU_ALTITUDE, displayValues.N_FCU_VS
	...
	fcu:setFcuDisplay({
		displayEnabled = power,
		speed = speedText,
		heading = headingText,
		altitude = digits(altitude, 5),
		verticalSpeed = vsText,
		spdMach = mach,
		spdManaged = displayValues.I_FCU_SPEED_MANAGED > 0,
...
GlobalFrameLoopManager:add(function()
	...
	if pressure == lastPressure and standard == lastStandard and inches == lastInches
		and qnh == lastQnh and power == lastPower then
		return
	end
